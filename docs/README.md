...
URPainter采用持续对话式的创作流程，通过LLM引导、SDM辅助和儿童互动三方协作完成创作。整个过程分为六个主要阶段（A-F），每个阶段都有明确的目标和切换条件。阶段转换系统是URPainter创作流程的"导航仪"，它引导整个对话从开启创意(A)，经历确定主题(B)、绘制主角(C)、联想拓展(D)、绘制辅助元素(E)，最终完成创作(F)。这种结构化的引导对于4-6岁儿童来说非常重要，能够将复杂的创作过程分解为可管理的小步骤。

关键机制：
阶段定义与描述：每个阶段(A-F)都有明确的目标、提示词模板和输出格式
转换条件判断：使用LLM智能判断当前对话是否满足进入下一阶段的条件
阶段流转控制：管理阶段间的顺序转换，确保创作流程的连贯性
提示词适配：根据当前阶段动态调整LLM的提示词，引导特定的创作行为
...
- **执行者**：LLM
- **具体流程**：
  1. 首先由LLM从预设问题库中随机选择问题对儿童进行提问
  2. 问题示例：
     - 今天你看到最酷的东西是什么？
     - 如果你能够发现一件特别的东西，它是什么？
     - 你最喜欢的动物是什么？为什么？
     - 如果你能变成任何东西，你会变成什么？
- **在这个阶段会到的值**：某一特定话题：系统存了几套预设好的话题的prompt，如果儿童的答案属于预设话题，则使用prompt作为此次对话的特定话题层(c)
- **切换条件**：获得儿童的明确回答作为关键词（主角）

#### B. 确立主题
- **目标**：通过对话确定创作主题
- **执行者**：LLM + 儿童
- **具体流程**：
  1. LLM基于儿童的关键词，引导儿童开联想
  2. 通过启发式对话扩展创意
  3. 引导儿童确定最终创作主题
- **切换条件**：达成明确且有趣的创作主题（围绕主角的某一具体事件n.+v.，或者是一个怎么样的主角adj.+n.），可能的格式类似于“吊机摘星星”"摩天大楼动物园“。

#### C. 绘制主角元素
- **目标**：完成画面主要元素（主角）的创作
- **执行者**：LLM + SDM + 儿童
- **具体流程**：
  1. LLM通过与儿童的多轮对话引导尽量详细的描述主角。等llm判定主角达到比较”丰满“的条件，即停止此轮引导，并向sdm发送经过总结的提示词。
  2. SDM生成3轮廓方案供儿童选择。
  3. 儿童选择并拖拽定位
  4. 儿童进行个性化上色
- **切换条件**：完成主要元素（主角）的创作和上色

#### D. 联想阶段
- **目标**：引导延伸创作内容
- **执行者**：LLM + 儿童
- **具体流程**：
  1. LLM基于已有元素引导场景其他内容的联想
  2. 启发儿童思考画面补充内容
- **切换条件**：产生新的创作元素想法关键词
...
- **具体流程**：
  ...
  - 但过程相对简化
  - 需考虑与主体的关系
- **循环**：返回联想阶段继续创作，直到完成3个元素绘制（包含主角），llm询问儿童是否完成创作，若儿童确认，则结束循环，进入阶段F，若儿童仍像继续绘画，则继续循环（—D-E-D-），直到儿童确认/同意完成创作。
...
LLM：超级长的胳膊真是个好主意！那么它是像一根大长竿，还是像一条巨大的蛇？
儿童: 像一条巨大的蛇！

LLM：为了更容易勾住星星，你觉得吊车的钩子应该怎样升级变得更强大？可以从钩子的数量和形状考虑哦！
儿童：钩子要很多像蜘蛛网一样，这样可以抓住很多星星！

LLM：蜘蛛网钩子！这真是一个很有创造力的想法！那么请选择一个最符合你心目中的神奇的吊机吧！
sdm：生成三种胳膊像弯曲的蛇、钩子像蜘蛛网的吊机图片。
儿童: 选择并拖拽放置
...
LLM：棉花糖云朵会让画面看起来更加柔和！选择你心目中的云朵吧！
SDM: 生成三种不同类型的云朵轮廓图。
儿童：选择并拖拽放置
...
在URPainter项目中，我们对LLM记忆功能进行了一系列优化，使AI助手能够更好地记住和利用对话历史。以下是主要改进：

### 1. 记忆格式化的增强

- **添加了时间戳**：每条消息都包含具体的时间信息，便于时间序列追踪
- **添加了消息编号**：使消息有明确的序列关系，便于引用
- **清晰的角色标识**：明确区分用户和AI助手的消息，提高可读性

### 2. 增加了记忆窗口大小

- 从原来的默认20条消息增加到30条
- 这使得LLM能够记住更长的对话历史，提高了长期记忆能力
- 在保持性能的同时，扩大了上下文理解范围

### 3. 增强了记忆检索能力

- **智能识别记忆查询**：通过关键词和模式匹配识别用户是否在询问历史信息
- **分类处理不同类型的记忆查询**：
  - 查询之前的消息（"我刚才说了什么"）
  - 查询特定主题（"关于吊机的"）
- **主题提取**：能够从用户问题中提取关键主题词

### 4. 专门的记忆提示增强

- **突出显示相关记忆**：当检测到记忆相关查询时，会特别强调相关的历史信息
- **增加记忆权重**：在提示词中给予记忆更高的权重，确保LLM注意到这些信息
- **添加记忆检索指令**：明确指导LLM如何使用和回应记忆相关的查询

### 5. 客户端存储实现

- **使用SessionStorage**：将对话历史存储在浏览器的会话存储中
- **会话级持久化**：确保在同一浏览器会话中记忆保持，但关闭浏览器后清除
- **按会话ID组织**：每个对话有独立的存储空间，避免混淆

### 6. 创意元素提取能力

- **自动提取创意元素**：从对话中识别主题、角色和其他元素
- **多模式匹配**：使用多种表达模式来提高提取准确性
- **避免重复**：确保提取的元素不重复

这些改进使得URPainter的AI助手能够更好地记住用户之前提到的信息（如吊机的颜色是红色），并在用户询问时正确回忆。这大大提高了对话的连贯性和用户体验，使AI助手更像一个有记忆能力的对话伙伴，而不仅仅是单次响应的工具。



1. 存储机制改进
从服务端内存迁移到SessionStorage：解决浏览器刷新/重启应用时自动清除记忆的需求
引入窗口管理机制：限制记忆中保存的对话轮数，提高性能和相关性
保留创意元素提取：继续保留现有代码中的创意元素提取功能
2. 架构优化
借鉴LangChain的模块化设计：分离记忆管理与存储机制
增加流程控制器(Controller)：明确数据流向，协调各组件之间的交互
引入回调机制：用于状态更新和事件处理
优化Builder-Manager关系：明确职责分离和协作方式
3. 相关模块适配
更新StageDetector/ThemeDetector：适配新的记忆获取方式
调整API接口：接收从前端传来的历史记录而非依赖服务端存储
适配PromptBuilder：确保提示词构建功能与新记忆系统兼容
4. 前端实现
封装SessionStorage交互：创建一套管理本地记忆的API
完整的消息处理流程：从用户输入到存储更新的完整链路
一致性管理：确保前后端共享相同的消息历史视图
5. 性能与体验提升
减轻服务器负担：通过客户端存储分担服务器压力
提高响应速度：通过记忆窗口管理优化LLM处理效率
会话隔离：每个浏览器会话独立管理记忆，符合用户预期


以下是一个分阶段实施的技术路线，按照依赖关系和复杂度排序：
阶段一：基础架构准备（基础设施层）
创建记忆控制器:
调整Storage类定位:
更新Manager接口
...
架构层：
# 完整提示词框架
1. [persona] 系统基础层 - 身份与角色
2. [context] 背景信息层 - 包含主题专业知识和对话历史
3. [task] 任务指令层 - 包含阶段引导
4. [format] 格式规范层 - 输出格式要求
5. [exemplar] 示例演示层 - 提供成功案例
6. [tone] 语气风格层 - 交流方式定义


两者采用采用"嵌套+标记"的组织方式：
a系统基础层 ≈ [persona] + 部分[tone]
b对话阶段层 ≈ [task] + 部分[format]
c特定话题层 ≈ 部分[context] + 部分[exemplar]
d上下文记忆层 ≈ 部分[context]
e当前输入层 - 这是最终的用户输入
