...
注意：简单判断用户是否提供了关于主角的足够细节即可。要求至少有2-3个不同方面的描述。
      ...
      case 'C1':
        transitionConditions = `
1. 用户是否明确提出了想要添加的新元素？
2. 这个元素是否足够具体，可以进入细节描述阶段？

注意：只要用户明确指出想要添加的具体元素（如"小狗"、"彩虹"等），就可以转换到细节描述阶段。
        `;
        break;
      
      case 'C2':
        transitionConditions = `
1. 用户是否已描述了当前元素的至少1个特征？
2. 当前元素的描述是否已经完成？

注意：如果用户已经提供了足够的细节描述，并且对话自然地可以转向讨论新元素，则可以转换。转换后要回到C1阶段询问是否要添加更多元素，除非用户明确表示完成。
        `;
        break;
...
2. 是否已经命名了作品或表达了类似结束的意思？

注意：这是最终阶段，主要关注用户是否已经表达满意或完成的意愿，不需要进一步转换。
        `;
        break;
